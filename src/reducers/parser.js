...
}else if(jObj[key]===null){val+=this.indentate(level)+'<'+key+'/'+this.tagEndChar;}else if(typeof jObj[key]!=='object'){//premitive type
...
var tagName='';for(;i<xmlData.length&&xmlData[i]!=='>'&&xmlData[i]!==' '&&xmlData[i]!=='\t';i++){tagName+=xmlData[i];}tagName=tagName.trim();//console.log(tagName);
 ...
 * @param {number} i
 */function readAttributeStr(xmlData,i){var attrStr='';var startChar='';for(;i<xmlData.length;i++){if(xmlData[i]===doubleQuote||xmlData[i]===singleQuote){if(startChar===''){startChar=xmlData[i];}else if(startChar!==xmlData[i]){//if vaue is enclosed with double quote then single quotes are allowed inside the value and vice versa
continue;}else{startChar='';}}else if(xmlData[i]==='>'){if(startChar===''){break;}}attrStr+=xmlData[i];}if(startChar!==''){return false;}return{value:attrStr,index:i};}/**
